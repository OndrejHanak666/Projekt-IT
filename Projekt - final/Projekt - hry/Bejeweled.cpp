    ...
    if (file.is_open())
    {
        ...
        for (int i = 1; i <= 8; i++)
            for (int j = 1; j <= 8; j++)
            {
                ...
                // Zajistí, že všechny ozdoby jsou správnì umístìny a nespojují se
                grid[i][j].match = 0;
            }
        file.close();
        return true;
...
void playBejeweled()
{
    ...
    {
        inMenu = false; // Pokud je save dostupný, pokraèujeme v uložené høe
    }
    ...
    while (app.isOpen())
    {
        ...
        while (app.pollEvent(e))
        {
            ...
            {
                if (e.key.code == Keyboard::Escape)         // Uživatelský návrat do hlavní nabídky
                {
